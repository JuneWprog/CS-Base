...
定义：安全的操作是指该操作不会更改服务器上的资源或状态。执行这种操作不会对资源产生任何副作用。

特点：安全操作应该是只读取数据，而不修改或删除数据。

举例：

GET：用来请求获取资源，不应修改服务器上的任何数据。

HEAD：用来获取资源的头部信息，与 GET 类似，但不返回实体内容。

3. 幂等性（Idempotent）
   
定义：幂等操作是指无论执行多少次，操作的结果都是相同的。即使对同一资源进行多次相同的操作，最终的结果不会改变。

特点：幂等操作不会造成副作用或产生不同的结果。

举例：

GET：获取资源是幂等的，因为多次请求同一资源的结果应该是相同的（即使在请求之间资源有所变化）。

PUT：更新资源的整个内容是幂等的，因为即使多次执行相同的更新请求，结果是一样的。

DELETE：删除资源是幂等的，因为删除一次资源和多次删除同一个资源的效果是相同的（资源被删除后，再次删除没有效果）。

例子说明

GET 方法：

安全：请求 GET /users/123 来获取用户 ID 123 的信息，这不会对用户数据产生任何更改。
幂等：多次请求 GET /users/123 都会返回相同的信息，前提是用户数据没有改变。
POST 方法：

不安全：POST /users 可能用来创建一个新用户，这会改变服务器上的资源（创建新记录）。

不幂等：多次执行相同的 POST /users 请求可能会创建多个用户记录，因此效果会有所不同。

PUT 方法：

不安全：PUT /users/123 用来更新用户 ID 123 的信息，这会改变服务器上的资源。
幂等：多次执行相同的 PUT /users/123 请求，结果都是将用户 ID 123 的信息更新为相同的状态。
DELETE 方法：

不安全：DELETE /users/123 用来删除用户 ID 123 的记录，这会改变服务器上的资源。
幂等：多次执行相同的 DELETE /users/123 请求，结果都是用户 ID 123 的记录被删除。
